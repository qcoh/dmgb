...
{
	u8 &operator[](const u16 addr) noexcept
	{
		return m_mmu[addr];
	}

	u8 read(const u16 addr) const noexcept override
	{
		return m_mmu[addr];
	}

	void write(const u16 addr, const u8 v) noexcept override
	{
		m_mmu[addr] = v;
	}
