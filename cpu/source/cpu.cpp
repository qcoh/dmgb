...
void ldi_hl_a(cpu& cpu, mmu_ref mmu) {
	mmu[cpu.hl] = cpu.a;
	cpu.hl = static_cast<u16>(cpu.hl + 1);
}

void ldd_hl_a(cpu& cpu, mmu_ref mmu) {
	mmu[cpu.hl] = cpu.a;
	cpu.hl = static_cast<u16>(cpu.hl - 1);
}

void ldi_a_hl(cpu& cpu, mmu_ref mmu) {
	cpu.a = mmu[cpu.hl];
	cpu.hl = static_cast<u16>(cpu.hl + 1);
}
...
void step(cpu& cpu, mmu_ref mmu) noexcept {
	...
	case 0x22: ldi_hl_a(cpu, mmu); break;
	...
	case 0x2a: ldi_a_hl(cpu, mmu); break;
	...
	case 0x32: ldd_hl_a(cpu, mmu); break;
